...
%\usetheme{metropolis}
\usetheme[sectionpage=simple, numbering=none, progressbar=head, background=light, block=transparent]{kami}
% \setbeamersize{text margin left=2.1ex,text margin right=2.1ex}
...
\definecolor{blue}{HTML}{306E7C}%#306E7C
\definecolor{red}{HTML}{FF5656}%#FF5656
\definecolor{green}{HTML}{60EA90}%#60EA90
\definecolor{orange}{HTML}{FFFC31}%#FFFC31
		...
		\begin{column}{0.40\textwidth}
			...
			\begin{center}
				\begin{tikzpicture}[
						start chain,
						node distance=5mm,
						every node/.style={draw,on chain,join,inner sep=0.6ex,inner xsep=1.3ex, fill=fg},
						every join/.style={stealth-stealth}
					]
					\node {\color{bg}\strut Host};
					\node {\color{bg}\strut FPGA};
					\node {\color{bg}\strut Spikey};
				\end{tikzpicture}
			\end{center}
		\end{column}
		\hfill
...
\begin{frame}{Task 3: bouncing vehicle}
	\begin{columns}[onlytextwidth]
	\begin{column}{0.58\textwidth}
		\begin{center}
			\begin{tikzpicture}[scale=1, transform shape]
				\node[circle,white,fill=red,text width=0.8cm,align=center] (stm_exc) at (0,0.7) {\scriptsize\strut Right};
				\node[circle,white,fill=blue,text width=0.8cm,align=center] (stm_inh) at (0,-0.7) {\scriptsize\strut Left};
				\node[circle,white,fill=red,text width=0.8cm,align=center] (mn) at (2,0) {\scriptsize\strut MN};
				\node[circle,white,fill=blue,text width=0.8cm,align=center] (inh_dummy) at (4,-0.7) {\scriptsize\strut $\cdot -1$};
				\node[circle,white,fill=red,text width=0.8cm,align=center] (bias) at (4,0.7) {\scriptsize\strut bias};
				\node[circle,white,fill=red,text width=0.8cm,align=center] (inh) at (6,0) {\scriptsize\strut $\sim$MN};
				\node[circle,white,fill=fg,text width=0.8cm,align=center] (motor) at (4,-2.7) {\scriptsize\strut Motor};

				\draw[-stealth,thick,red] (stm_exc) -- (mn);
				\draw[-stealth,thick,blue] (stm_inh) -- (mn);
				\draw[-stealth,thick,red] (mn) to[out=120,in=180] (2,1.5) to[out=0,in=60,-stealth] (mn);
				\draw[-stealth,thick,red] (mn) -- (inh_dummy);
				\draw[-stealth,thick,red] (bias) -- (inh);
				\draw[-stealth,thick,blue] (inh_dummy) -- (inh);
				\draw[-stealth,thick,red] (mn) -- (motor);
				\draw[-stealth,thick,red] (inh) -- (motor);
			\end{tikzpicture}
		\end{center}
	\end{column}
	\hfill
	\begin{column}{0.38\textwidth}
		\begin{itemize}
			\item transient right/left stimulus sets/resets the state of the motor neuron (MN) population
			\item MN and $\sim$MN determine the motor direction
		\end{itemize}
	\end{column}
	\end{columns}
\end{frame}
